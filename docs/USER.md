...
There are five preset buttons (**$25**, **$50**, **$100**, **$250**, **$500**) and an **Other** option. Selecting a preset fills in the amount automatically; selecting **Other** lets the donor type any amount between **$10.00** and **$10,000.00** (inclusive). The amount field accepts whole dollars or two decimal places (for example, `75` or `75.50`). Other formats &mdash; scientific notation, commas, currency symbols, or negative numbers &mdash; are rejected at submission with a clear error.
...
If something prevents a checkout from being created, the donor sees a dedicated error page with a heading appropriate to the situation and a reassuring note:
...
- **Input problem (400 / 422)** &mdash; the donor's name, email, or amount did not pass validation, or the page's session expired before submission. Refreshing the page and re-submitting is usually sufficient.
...
- The donor's name, email, and phone number are stored alongside the Stripe payment record for receipt and reconciliation purposes only.
