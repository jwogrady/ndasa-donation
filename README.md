...
A webhook-authoritative online donation application for the **NDASA Foundation**, built on Stripe Checkout.
...
- **One-time donations** &mdash; preset amounts ($25 / $50 / $100 / $250 / $500) and a free-form custom amount, bounded by configurable minimum and maximum.
- **Optional fee cover** &mdash; donor can gross up the charge so the foundation nets the intended amount after Stripe's 2.9% + $0.30.
- **Multiple payment methods** &mdash; Card, Link, Apple Pay, Google Pay, and ACH Direct Debit surface automatically based on the foundation's Stripe dashboard settings.
- **Webhook-verified records** &mdash; `checkout.session.completed`, `.async_payment_succeeded`, `.async_payment_failed`, `charge.refunded`, `payment_intent.payment_failed`. Signatures are verified with a 300-second tolerance; duplicate deliveries are idempotent.
- **Admin panel** at `/admin` with HTTP Basic Auth. Includes a metrics dashboard, a `.env` config editor (atomic write, CSRF-protected), and a System Health panel covering database, environment writability, and required configuration.
...
- Credentials set via `ADMIN_USER` and `ADMIN_PASS` in `.env`.
...
- Optional `APP_VERSION` in `.env` shows in the admin footer; if unset the app falls back to the short git hash, then a hardcoded constant.
